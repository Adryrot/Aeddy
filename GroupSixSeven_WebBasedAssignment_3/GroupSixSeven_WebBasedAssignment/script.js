...
// Sound notifications
let bellSound = new Audio('bell_ring.mp3');
let fahhSound = new Audio('FAHH.mp3');
...
function initializePomodoro() {
    ...
    // Initialize stop button visibility for default focus session
    document.getElementById('stop-btn').classList.remove('hidden');
    document.getElementById('stop-btn').disabled = true; // Initially disabled
}
...
function showCycleMode() {
    ...
    document.getElementById('stop-btn').classList.remove('hidden');
    document.getElementById('stop-btn').disabled = !isTimerActive; // Set based on timer state
}
...
function hideCycleMode() {
    ...
    // Show stop button for focus session
    if (sessionType === 'focus') {
        document.getElementById('stop-btn').classList.remove('hidden');
        document.getElementById('stop-btn').disabled = !isTimerActive; // Set based on timer state
    } else {
...
function startTimer() {
    ...
    document.getElementById('stop-btn').disabled = false; // Enable stop button
...
function handleTimerComplete() {
    ...
    if (isInCycleMode) {
        if (currentCyclePhase === 'focus') {
            // Focus phase complete - play bell sound IMMEDIATELY
            playSound(bellSound);
            ...
            // Alert AFTER a short delay to let sound play
            setTimeout(() => {
                alert('Focus session complete! Starting short break.');
                startTimer(); // Auto-start break
            }, 100);
        } else {
            // Break phase complete - play fahh sound IMMEDIATELY
            playSound(fahhSound);
            
            // Handle cycle complete after short delay
            setTimeout(() => {
                handleCycleComplete();
            }, 100);
        }
    } else if (sessionType === 'focus') {
        // Regular focus session complete - play bell sound IMMEDIATELY
        playSound(bellSound);
        ...
        // Break sessions complete - play fahh sound IMMEDIATELY
        playSound(fahhSound);
...
function stopTimer() {
    ...
    document.getElementById('stop-btn').disabled = true; // Disable stop button when timer stops
}
...
function stopTimerAndCount() {
    ...
    if (isInCycleMode) {
        ...
        if (currentCyclePhase === 'focus') {
            // Stop button clicked during focus phase - play bell sound IMMEDIATELY
            playSound(bellSound);
            ...
            // Stop button clicked during break phase - play fahh sound IMMEDIATELY
            playSound(fahhSound);
        ...
        // Regular focus session - play bell sound IMMEDIATELY
        playSound(bellSound);
